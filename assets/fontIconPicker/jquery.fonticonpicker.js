...
(function ($) {
    ...
    function Plugin(element, options) {
        ...
        this.iconPicker = $('<div/>', {
            ...
            '<input type="text" name="" value="" placeholder="Search possible icons by name" class="icons-search-input"/>' +
    ...
    Plugin.prototype = {
        ...
        renderIconContainer: function () {
            ...
            for (var i = 0, item; item = iconsPaged[i++];) {
                ...
                // Set the icon box
                $('<span/>', {
                    html: '<i data-fip-value="' + item + '" ' + (this.settings.useAttribute ? (this.settings.attributeName + '="' + ( this.settings.convertToHex ? '&#x' + parseInt(item, 10).toString(16) + ';' : item ) + '"') : 'class="icon ' + item + '"') + '></i>',
                    'class': 'fip-box',
                    title: flipBoxTitle
        ...
        setSelectedIcon: function (theIcon) {
            ...
            if (this.settings.useAttribute) {
                if (theIcon) {
                    this.iconPicker.find('.selected-icon').html('<i ' + this.settings.attributeName + '="icon ' + ( this.settings.convertToHex ? '&#x' + parseInt(theIcon, 10).toString(16) + ';' : theIcon ) + '"></i>');
                } else {
        ...
        toggleIconSelector: function () {
            ...
            this.iconPicker.find('.selector-button i').toggleClass('caret down');
            this.iconPicker.find('.selector-button i').toggleClass('caret up');
            if (this.open) {
