...
<feed xmlns="http://www.w3.org/2005/Atom">
    ...
    @foreach($items as $item)
        <entry>
            <author>
                <name><![CDATA[{{ $item->seller_name }}]]></name>
            </author>
            <title type="html"><![CDATA[{{ $item->title }}]]></title>
            <link rel="alternate" type="text/html" href="{{ route('items.show', $item->slug) }}"></link>
            <id>{{ $item['link'] }}</id>
            <summary type="html"><![CDATA[{{ $item->description }}]]></summary>
            <updated>{{ $item->updated_at->toDayDateTimeString() }}</updated>
        </entry>
    @endforeach
</feed>